  ...
  let module Machine = struct
    ...
    let create (scope : Scope.t) { I.clock; clear; data_in_valid; data_in } =
      ...
      let router =
        Router.hierarchical
          scope
          { Router.I.clock = { clock; clear }
          ; up = dn
          ; dns = [ { tready = pulse_1_ready }; { tready = pulse_2_ready } ]
          }
      in
      let pulse_1 =
        Pulse.hierarchical
          scope
          { Pulse.I.clock = { clock; clear }; up = List.nth_exn router.dns 0 }
      in
      let pulse_2 =
        Pulse.hierarchical
          scope
          { Pulse.I.clock = { clock; clear }; up = List.nth_exn router.dns 1 }
      in
